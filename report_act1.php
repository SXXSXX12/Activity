...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
	global $e_page;
	global $querystr;
        $act_id = $_GET['act_id'];//ต้องรับใน function ด้วย
	...
	$lt_page=$total_p-4;
	if($chk_page>0){  
		echo "<a  href='$urlfile?act_id=$act_id&s_page=$pPrev".$querystr."' class='naviPN'>Prev</a>";
	}
	for($i=$total_start_p;$i<$total_end_p;$i++){  
		$nClass=($chk_page==$i)?"class='selectPage'":"";
		if($e_page*$i<=$total){
		echo "<a href='$urlfile?act_id=$act_id&s_page=$i".$querystr."' $nClass  >".intval($i+1)."</a> ";   
		}
	}		
	if($chk_page<$total_p-1){
		echo "<a href='$urlfile?act_id=$act_id&s_page=$pNext".$querystr."'  class='naviPN'>Next</a>";
	}
}   

                
                $q = "SELECT s.std_code,CONCAT(s.fname,' ',s.lname) AS fullname
                FROM history_act h
                ...
                <table width='100%' align='center' id="example3">
                    ...
                    while ($rs = mysqli_fetch_array($qr)) {
                        ...
                        <tr>
                            ...
                            <td align='center'><a href='reportstd_act.php?std_code=<?= $rs['std_code'] ?>'><img src="images/do.png" width="35" height="35"></a></td>
                        </tr>
                    <?php } ?>
             ...
             <table width='100%'>
                 <tr>
                    ...
                    <tr>
                         ...
                         <td align='center'><?php echo !empty($rs1['men'])? round(($rs1['men']*100)/($rs1['men']+$rs1['women']),2).'%':''; ?></td>
                    </tr>
                    <tr>
                        <td align='center'>เพศหญิง</td>
                        <td align='center'><?php echo $rs1['women']; ?></td>
                        <td align='center'><?php echo !empty($rs1['women'])? round(($rs1['women']*100)/($rs1['men']+$rs1['women']),2).'%':''; ?></td>
                    </tr>
                    <tr>
