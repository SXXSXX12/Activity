                            ...
                            <div class="row uniform 50%">
                                ...
                                <div class="6u ">
                                    <select name ="take_hour" id="take_hour" value="<?= isset($_GET['method'])?$rs['start_time']:''?>" required>
                                        ...
                                        <?php
                                        for ($i = 0; $i <= 23; $i++) {
                                            if($i== substr($rs['start_time'], 0,2)){$select='selected';}else{$select='';}
                                            if ($i < 10) {
                                ...
                                <div class="6u ">
                                    <select name ="take_minute" id="take_minute">
                                        ...
                                        <?php
                                        for ($i = 0; $i <= 59; $i++) {
                                            if($i== substr(isset($_GET['method'])?$rs['start_time']:'', 3,2)){$select='selected';}else{$select='';}
                                            if ($i < 10) {
                                                echo "<option value='0" . $i . "'".$select.">0" . $i . "</option>";
                                            } else {
                                                echo "<option value='" . $i . "'".$select.">" . $i . "</option>";
                                    ...
                                    <div class="12u">
                                        ...
                                        <div class="row uniform 50%">
                                            <div class="12u 12u(narrower)">
                                                ...
                                                $ind = 11;
                                                for($i=1;$i<=4;$i++){ 
                                                    if(isset($_GET['method'])?$rs[$ind]:''==1){ $check="name='year".$i."' checked";}else{$check="name='year".$i."'";}
                                                    ?>
                                        ...
                                        <select name='teacher' required>
                                           ...
                                           $sql = mysqli_query($conn, "SELECT teach_id,CONCAT(teach_name,' ',teach_lname) as fullname FROM teacher ORDER BY fullname ASC");
                                            while ($teach = mysqli_fetch_assoc($sql)) {
                                                if($teach['teach_id'] ==$rs['respon'] ){$select='selected';}else{$select='';}
